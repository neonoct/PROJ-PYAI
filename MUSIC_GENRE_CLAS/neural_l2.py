...
# Define a function to create the model with L2 regularization and learning rate scheduling
def create_model(optimizer='adam', neurons=128, dropout_rate=0.3, l2_penalty=0.001):
    model = Sequential([
        Dense(neurons, activation='relu', kernel_regularizer=l2(l2_penalty), input_shape=(X_train.shape[1],)),
        BatchNormalization(),
        Dropout(dropout_rate),
        Dense(neurons, activation='relu', kernel_regularizer=l2(l2_penalty)),
        BatchNormalization(),
        Dropout(dropout_rate),
        Dense(neurons, activation='relu', kernel_regularizer=l2(l2_penalty)),
        BatchNormalization(),
...
# Fit the random search model with learning rate reduction callback
random_search_result = random_search.fit(X_train, y_train_categorical, callbacks=[reduce_lr])
